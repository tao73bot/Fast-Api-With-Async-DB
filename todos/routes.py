...
async def update_todo(todo_id: str, todo: TodoUpdate, db: AsyncSession = Depends(get_db)):
    updated_todo = await service.update_todo(todo_id, todo, db)
...
async def delete_todo(todo_id: str, db: AsyncSession = Depends(get_db)):
    deleted_todo = await service.delete_todo(todo_id, db)
